...
open class GANormalizePickerView: UIView {
    ...
    var config: GANormalizePickerViewConfig?
    
    typealias PickerViewDidSelectHandler = (_ row: Int, _ component: Int, _ type: GANormalizePickerViewDataType) -> ()
    var pickerViewDidSelectHandler: PickerViewDidSelectHandler?
    
    typealias PickerViewMultigroupDidSelectHandler = (_ selected: [Int : Int], _ type: GANormalizePickerViewDataType) -> ()
    ...
    @IBAction func cancle(_ sender: UIButton) {
        pickerViewDidSelectHandler?(myRow, myComponent, .cancle)
        pickerViewMultigroupDidSelectHandler?([0:0], .cancle)
    }
    
    @IBAction func confirm(_ sender: UIButton) {
        pickerViewDidSelectHandler?(myRow, myComponent, self.dataType)
        pickerViewMultigroupDidSelectHandler?(selectedData, .cancle)
    }
...
extension GANormalizePickerView: UIPickerViewDelegate, UIPickerViewDataSource {
    public func pickerView(_ pickerView: UIPickerView, widthForComponent component: Int) -> CGFloat {
        return CGFloat(300 / dataSource.count)
    }
    ...
    //        let fontSize: CGFloat = self.type == .blgs ? 9.0 : 14.0
    //        print(fontSize)
    //        let attributed = NSMutableAttributedString(string: dataSource[component][row])
    //        attributed.addAttribute(NSAttributedStringKey.font, value: UIFont.systemFont(ofSize: fontSize), range: NSMakeRange(0, attributed.length))
    //        attributed.addAttribute(NSAttributedStringKey.foregroundColor, value: UIColor.black, range: NSMakeRange(0, attributed.length))
    //        return attributed
    //    }
