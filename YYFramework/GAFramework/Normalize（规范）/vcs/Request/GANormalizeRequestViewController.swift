...
//      let CheckSum = ga_hexStringFromString(s: (Secret + Nonce + CurTime).sha1())
        ...
        print(header)
        ZHReuqest.share.request(baseUrlType: .url, zhUrlApi: ZHRequestApi.IM.rawValue, method: .post, parameters: ["accid":"abc13A14aeq", "mobile":"13146218617"], headers: header, successHandler: { (model) in
            print(model.message)
    ...
    func binary2dec(num:String) -> Int {
        var nums = String(num.filter { num.contains($0) })
        var sum = 0
        for c in nums {
            print(c)
            sum = sum * 2 + Int("\\(c)")!
