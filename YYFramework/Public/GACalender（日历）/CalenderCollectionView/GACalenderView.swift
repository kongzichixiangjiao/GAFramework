...
class GACalenderView: UIView {
    ...
    @objc private func _collectionViewPanAction(sender: UIPanGestureRecognizer) {
        ...
        let yV = sender.velocity(in: collectionView).y
        let xT = sender.translation(in: collectionView).x
        let yT = sender.translation(in: collectionView).y
    ...
    private func _setupControlerHeaderView(year: Int, month: Int) {
        controlHeaderView.ymLabel.text = "\(year)-\(month)"
    }
