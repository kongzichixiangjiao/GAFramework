...
class YYBaseViewController: UIViewController {
    ...
    public var kNavigationViewMaxY: CGFloat {
        if isShowNavigationView {
            return kNavigationViewHeight + kNavigationViewTop
        } else {
            return kNavigationViewTop
        }
    }
    ...
    private func initNavigationView(title: String) {
        if isShowNavigationView {
            ...
            navigationView.addConstraint(NSLayoutConstraint(item: navigationView, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .height, multiplier: 1, constant: kNavigationHeight + kNavigationViewTop))
...
public extension NSObject{
    public class var base_nameOfClass: String {
        // NSStringFromClass(self) 获得的是项目名.类名
        return NSStringFromClass(self).components(separatedBy: ".").last!
    }
    
    public var base_nameOfClass: String{
