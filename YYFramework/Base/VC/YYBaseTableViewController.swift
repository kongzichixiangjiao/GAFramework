...
class YYBaseTableViewController: YYBaseViewController {
    ...
    lazy var tableView: UITableView = {
        let t = UITableView(frame: CGRect.zero, style: tableViewStyle)
        t.delegate = self
        t.dataSource = self
    ...
    public func base_initTableView() {
        ...
        if isShowTopSave {
            if #available(iOS 11.0, *) {
                self.view.addConstraint(NSLayoutConstraint(item: self.tableView, attribute: .top, relatedBy: .equal, toItem: self.view, attribute: .top, multiplier: 1, constant: kNavigationViewHeight + UIApplication.shared.windows[0].safeAreaInsets.top))
            } else {
                self.view.addConstraint(NSLayoutConstraint(item: self.tableView, attribute: .top, relatedBy: .equal, toItem: self.view, attribute: .top, multiplier: 1, constant: kNavigationViewHeight + 20))
            }
        } else {
            self.view.addConstraint(NSLayoutConstraint(item: self.tableView, attribute: .top, relatedBy: .equal, toItem: self.view, attribute: .top, multiplier: 1, constant: kNavigationViewHeight))
        }
